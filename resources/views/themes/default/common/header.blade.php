...
<html>
  ...
  <body>
    ...
    <!-- end preloader -->
    <!-- Header start -->
    <div class=""uk-sticky="start: 100; animation: uk-animation-slide-top; cls-active:uk-navbar-sticky; sel-target:.uk-navbar-container; class:uk-sticky;">
      <!-- uk-scrollspy="target: [uk-scrollspy-class], .uk-navbar, li; cls: uk-animation-slide-bottom-small; delay: 50; repeat: false;" -->
      <!-- Main Menu -->
      <div class="uk-visible@l  uk-navbar-container text-white uk-box-shadow-medium uk-main-header-transparent uk-navbar-transparent">
          ...
          <nav class="uk-navbar d-flex uk-flex-middle uk-flex-between" uk-navbar="  uk-dropbar-top">
            ...
            <div>
              <ul class="uk-navbar-nav uk-position-relative" style="margin-left:auto;">
                ...
                <li>
                  <a href="{{ route('page.posttype_detail', $about_us->uri) }}">{{$about_us->post_type}} </a>
                </li>
                <!-- Begin: expeditions -->
                            <li>
               ...
               <!-- </li>-->
                ...
                <!-- Begin: trekkings  -->
                <li>
                  <a href="{{ route('page.posttype_detail', $nav_trekkings->uri) }}">{{$nav_trekkings->post_type}} <span uk-navbar-parent-icon></span>
                  </a>
                  <div uk-drop="boundary:.uk-navbar; stretch: true; flip: false; delay-hide: 20;" class="uk-dropdown">
                    <div class="">
                      <div class=" uk-grid-large tabs" uk-grid>
                        ...
                        @foreach ($regions as $row)
							@if (tripbyregions($row->id)->count() > 0)
                          <!--  -->
                          <div id="{{$row->uri}}" class="{{$loop->first?'tab-contentT uk-active':'tab-contentT'}}">
                            <div class="uk-child-width-1-3 uk-grid-medium" uk-grid>
                              @if(tripbyregions($row->id)->count()>0)
                            ...
                            <a href="{{ route('page.regionlist', $row->uri) }}" class="uk-align-right uk-margin-small-top uk-margin-remove-bottom uk-button uk-button-text ">View All</a>
                          </div>
                          <!--  -->
                          @endif
                          @endforeach
            ...
            <div class="">
              <div class="uk-flex uk-flex-middle  uk-grid-medium">
                ...
                @if($contact_us != NULL)
                <div>
                  <a href="{{ route('page.posttype_detail', $contact_us->uri) }}" class="uk-button button-primary button-primary-active"> {{$contact_us->post_type}}</a>
                </div>
                @endif
              </div>
            </div>
            ...
            <nav class="uk-navbar" uk-navbar="{&quot;container&quot;:&quot;.uk-header-mobile&quot;}">
              ...
              <div class="uk-navbar-right">
                <div class="uk-navbar-item">
                  <div class="uk-visible@s uk-grid-expand uk-child-width-1-1 uk-margin-remove-vertical uk-grid uk-grid-stack" uk-grid="">
              ...
              <div class="uk-grid uk-child-width-1-1 uk-grid-stack" uk-grid="">
                <div>
                  <div class="uk-panel" id="module-menu-dialog-mobile">
                    <ul class="uk-nav uk-nav-primary  uk-nav-divider uk-nav-accordion uk-canvas" uk-nav="targets: > .js-accordion">
                      <li>
                        <a href="{{url('/')}}">Home</a>
                      </li>
                      ...
                      <li class="js-accordion uk-parent">
                        <a href="{{ route('page.posttype_detail', $nav_expeditions->uri) }}" aria-expanded="false">{{$nav_expeditions->post_type}}<span uk-nav-parent-icon="" class="uk-icon uk-nav-parent-icon"></span>
                        </a>                       
                        <ul class="uk-nav-sub">
                      ...
                      @if($regions->count()>0)
                      <li class="js-accordion uk-parent">
                        <a href="{{ route('page.posttype_detail', $nav_trekkings->uri) }}" aria-expanded="false">{{$nav_trekkings->post_type}} <span uk-nav-parent-icon="" class="uk-icon uk-nav-parent-icon"></span>
                        </a>
                        <ul class="uk-nav-sub">
                          @foreach ($regions as $row)
                            @if(tripbyregions($row->id)->count()>0)
                          <li>
                            <a href="{{ route('page.regionlist', $row->uri) }}">{{$row->title}}</a>
                          </li>
                          @endif
                          @endforeach
                        </ul>
